...
-- module-loading cleanup, best-effort editor-state cleanup, and small utilities
-- used by per-plugin test helpers.
local M = {}

--- Cleanup lua package.loaded for plugin.
--- This does nothing if the NTF_DISABLE_CLEANUP environment variable exists.
--- @param plugin_name string: lua module name (`lua/{plugin_name}/*.lua`)
function M.cleanup_loaded_modules(plugin_name)
  if os.getenv("NTF_DISABLE_CLEANUP") then
    return
  end
...
--- Cleanup editor state. (best effort)
---   - buffer, message, abbreviation, highlight, jump, history,
---     command, keymap, autocmd/augroup, option (exclude 'runtimepath'), arglist
--- Not supported: user defined function, `:filetype on`, `:syntax on`,
---   variable (g:, v:), register
--- This does nothing if the NTF_DISABLE_CLEANUP environment variable exists.
